...
busyCheck
	...
	AND 	R2, R3
	CMP 	R2, #0x10
	BEQ 	busyCheck	; check bit 4 is busy/high (1), if yes- check SR_R again
	LDR		R3, =DC_COMMAND
	LDR 	R1, =DC
	MOV 	R2, R3
	STRB 	R2, [R1]
	; write command to SSIO_DR_R
	LDR 	R1, =SSI0_DR_R
	STRB 	R0, [R1]
	; repeat step 1 & 2
	LDR 	R1, =SSI0_SR_R
...
busyCheck2
	...
	AND 	R2, R3
	CMP 	R2, #0x10
	BEQ 	busyCheck2	; check bit 4 is busy/high (1), if yes- check SR_R again
...
TNFcheck
	...
	AND 	R2, R3
	CMP 	R2, #0x02
	BNE 	TNFcheck ; check bit 1 is low(0), if yes- check SR_R again
	LDR		R3, =DC_DATA
	LDR 	R1, =DC
	STRB 	R3, [R1]
	; write command to SSIO_DR_R
